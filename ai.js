...
* ai.js
*
* Quoridor（クオリドル）ゲームのAI（人工知能）ロジックを実装。
* モンテカルロ木探索（MCTS）などを用いて最善手を探索。
* 難易度やシミュレーション回数の調整も可能。
*/

// PawnPositionのクローンを作成する関数
...
// 自分のターンでないプレイヤーのポーンのインデックスを返す関数
Game.prototype.getOpponentPawnIndex = function() {
    return (this._turn + 1) % 2;
}

// 現在のゲーム状態における有効な次のポーンの位置のタプルの配列を取得する関数
Game.prototype.getArrOfValidNextPositionTuples = function() {
    return indicesOfValueIn2DArray(this.validNextPositions, true);
}

// 自分のターンでないプレイヤーの次の有効なポーンの位置のタプルの配列を取得する関数
Game.prototype.getOpponentArrOfValidNextPositionTuples = function() {
    const opponentPawnIndex = this.getOpponentPawnIndex();
    return indicesOfValueIn2DArray(this.validNextPositions.map(row => row.map(cell => cell[opponentPawnIndex])), true);
}
...
// 自分のターンでないプレイヤーの次の有効な水平壁の位置を取得する関数
Game.prototype.getOpponentArrOfValidNoBlockNextHorizontalWallPositions = function() {
    const opponentPawnIndex = this.getOpponentPawnIndex();
    const nextHorizontals = indicesOfValueIn2DArray(this.validNextWalls.horizontal.map(row => row.map(cell => cell[opponentPawnIndex])), true);
    const noBlockNextHorizontals = [];
    for (let i = 0; i < nextHorizontals.length; i++) {
        if (this.testIfExistPathsToGoalLinesAfterPlaceHorizontalWall(nextHorizontals[i][0], nextHorizontals[i][1])) {   
            noBlockNextHorizontals.push(nextHorizontals[i]);
...
// 自分のターンでないプレイヤーの次の有効な垂直壁の位置を取得する関数
Game.prototype.getOpponentArrOfValidNoBlockNextVerticalWallPositions = function() {
    const opponentPawnIndex = this.getOpponentPawnIndex();
    const nextVerticals = indicesOfValueIn2DArray(this.validNextWalls.vertical.map(row => row.map(cell => cell[opponentPawnIndex])), true);
    const noBlockNextVerticals = [];
    for (let i = 0; i < nextVerticals.length; i++) {
        if (this.testIfExistPathsToGoalLinesAfterPlaceVerticalWall(nextVerticals[i][0], nextVerticals[i][1])) {
            noBlockNextVerticals.push(nextVerticals[i]);
...
// 自分のターンでないプレイヤーの次の有効な水平壁の位置を取得する関数
Game.prototype.getOpponentArrOfProbableValidNoBlockNextHorizontalWallPositions = function() {
    const opponentPawnIndex = this.getOpponentPawnIndex();
    const nextHorizontals = indicesOfValueIn2DArray(this.probableValidNextWalls.horizontal.map(row => row.map(cell => cell[opponentPawnIndex])), true);
    const noBlockNextHorizontals = [];
    for (let i = 0; i < nextHorizontals.length; i++) {
        if (this.testIfExistPathsToGoalLinesAfterPlaceHorizontalWall(nextHorizontals[i][0], nextHorizontals[i][1])) {   
            noBlockNextHorizontals.push(nextHorizontals[i]);
...
// 自分のターンでないプレイヤーの次の有効な垂直壁の位置を取得する関数
Game.prototype.getOpponentArrOfProbableValidNoBlockNextVerticalWallPositions = function() {
    const opponentPawnIndex = this.getOpponentPawnIndex();
    const nextVerticals = indicesOfValueIn2DArray(this.probableValidNextWalls.vertical.map(row => row.map(cell => cell[opponentPawnIndex])), true);
    const noBlockNextVerticals = [];
    for (let i = 0; i < nextVerticals.length; i++) {
        if (this.testIfExistPathsToGoalLinesAfterPlaceVerticalWall(nextVerticals[i][0], nextVerticals[i][1])) {
            noBlockNextVerticals.push(nextVerticals[i]);
...
class AI {
    /**
     * AIのインスタンスを生成
     * @param {number} numOfMCTSSimulations シミュレーション回数
     * @param {number} uctConst UCT定数
     * @param {boolean} aiDevelopMode 開発モード
     * @param {boolean} isWorker ワーカースレッド用か
     */
    constructor(numOfMCTSSimulations, uctConst, aiDevelopMode = false, isWorker = false) {
    ...
    chooseNextMove(game, forceMCTS = false) {
        ...
        // --- ここで候補手リストを保存 ---
        // Web Worker用: 候補手リストをpostMessageで返す
    ...
    static getRandomShortestPathToGoal(pawn, game) {
        ...
        while (queue.length > 0) {
            ...
            closedSet[position.row][position.col] = true;
            for (let i = 0; i < pawnMoveTuples.length; i++) {
